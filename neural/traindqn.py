...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="train network using dqn for 2048 game"
    )
    ...
    parser.add_argument(
        "--eps-start", type=float, default=0.9, help="epsilon start value"
    )
    parser.add_argument("--eps-end", type=float, default=0.05, help="epsilon end value")
    parser.add_argument(
        "--eps-decay", type=float, default=0.5, help="epsilon decay rate"
    )
    parser.add_argument(
        "--tau", type=float, default=1e-4, help="target network update rate"
    )
    parser.add_argument("--lr", type=float, default=1, help="learning rate")
    parser.add_argument(
        "--replay-memory-size", type=int, help="capacity of replay memory"
    )
    parser.add_argument(
        "-v",
        "--verbose",
        action="store_true",
        help="verbose mode, show result when training",
    )
    parser.add_argument(
        "-i",
        "--interval",
        type=int,
        default=16,
        help="interval to logging, no log if 0",
    )
        ...
        for t in count():
            ...
            if terminated or truncated:
                ...
                if args.verbose:
                    plot_result()
                    if (
                        (i_episode + 1) % LOG_INTERV == 0
                        and i_episode != 0
                        and LOG_INTERV != 0
                    ):
                        print(
                            f"Episode {i_episode + 1} finished, total reward: {total_reward}"
                        )
                break
